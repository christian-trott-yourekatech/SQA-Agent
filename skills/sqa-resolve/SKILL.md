...
2. Read `.claude/skills/sqa-resolve/project.md`. Extract the
   quality-check command from its "Quality-check command" section. Note
   any project-specific conventions for use later.

3. Run that command via Bash **once** to establish a clean baseline.
...
4. If the check fails at baseline, **stop and surface the failures to
