...
1. **Verify `sqa-tool` is installed.** Run `command -v sqa-tool` via
   Bash. If it's not on PATH, tell the user to install it:

   ```
   uv tool install --reinstall git+https://github.com/christian-trott-yourekatech/SQA-Agent.git
   ```

   …and stop. Don't try to work around a missing tool.

2. **Read project configuration.** Read
...
3. **Pre-review quality check.** Run the quality-check command via Bash,
...
4. **Start a new result file.** Run `sqa-tool start-result`. The first
   ...
   **If `start-result` fails with "unresolved findings,"** that means
   the previous session was left mid-flight (open findings still in
   the result file). Surface the situation to the user — show them
   the prior session's count via `sqa-tool status` — and ask whether
   to start a fresh session anyway (running `sqa-tool start-result
   --force`) or to first complete the prior session with
   `/sqa-resolve`. Do **not** silently pass `--force`; this guard
   exists to catch a real footgun and the user's intent should be
   confirmed.

5. **Pick parallelism.** Ask the user (in chat) what `max_agents` to use,
   or use `4` as the default if running non-interactively. Higher =
   faster but more aggressive on quota.

6. **Drive the bounded review loop.** Repeat:
...
7. **Report summary.** Run `sqa-tool status` and present the output
   to the user.
