...
- **Do NOT call `sqa-tool start-result`.** Your parent skill already
  started (or is operating on) the session's result file. Starting a
  new one would orphan every other resolve in this pass. The CLI now
  refuses this case, but the rule is here so you don't try.
- **Do not call `sqa-tool record-finding`** to capture incidental issues
  you notice. Save those for the next review pass — the safety guard on
  `record-finding` would refuse anyway once any finding in the active
  result is resolved.
- **Do not call `mark-reviewed`.** That's the review skill's job.
