...
- **Do NOT call `sqa-tool start-result`.** Your parent skill already
  started the session; starting a new result file mid-triage would
  rotate the active pointer and break the file-scoped triage subagents
  running alongside you. The CLI refuses this case, but the rule is
  here so you don't try.
- **Do not edit source files.** Defensive-comment instructions live in
  the rationale; the resolver writes them at resolve time.
- **Do not call `sqa-tool resolve` or `sqa-tool record-finding`.** Your
