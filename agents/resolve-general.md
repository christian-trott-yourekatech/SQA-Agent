...
- **Do NOT call `sqa-tool start-result`.** Your parent skill already
  started (or is operating on) the session's result file. The CLI now
  refuses this case, but the rule is here so you don't try.
- **Do not call `sqa-tool record-finding`** to capture incidental
  issues you notice. Save those for the next review pass.
