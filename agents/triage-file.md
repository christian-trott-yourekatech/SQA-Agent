...
- **Do NOT call `sqa-tool start-result`.** Your parent skill already
  started the session; calling `start-result` again would rotate the
  active result file out from under every other subagent. The CLI now
  refuses this case, but the rule is here so you don't try.
- **Do not edit source files.** Defensive-comment instructions live in
  the rationale; the resolver writes them at resolve time. This is
  what makes parallel triage safe.
